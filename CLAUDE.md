...
Checkpoint 4.7 complete. The fire sound (`public/sounds/pistol_fire.wav`) and the zombie growl/death sounds (`public/sounds/zombie_growl.wav`, `zombie_death.wav`) are all synthesized placeholders (Node-generated tones), not real recordings — swap them for real audio in a later checkpoint (9, ambience/music, is the natural point to revisit all placeholder audio).

Note: the original checkpoint 4 scope (state machine + line-of-sight + sounds) didn't explicitly cover damage/kill mechanics, and no later checkpoint claimed them either, so they were added at checkpoint 4 rather than left to silently expand checkpoint 7's scope. Player death is now fully playable end-to-end: health hits 0 → `playerState` flips to `"dead"` → pointer lock releases → HUD shows a death panel (score + Respawn/Main Menu buttons) → clicking either restores full health, alive state, and spawn position, and gameplay resumes with no page reload. What's still deferred: score does NOT reset on respawn (that's round/mode logic, checkpoint 7+), and "Main Menu" is currently just an alias for "Respawn" (checkpoint 9 gives it real behavior). See the decisions log and "Future mechanics" below.
...
- `PlayerState.respawn()` resets health/lifecycle only (`playerHealth` back to `PLAYER_MAX_HEALTH`, `playerState` back to `"alive"`); it deliberately does not touch `score`/`pointsBalance` — only checkpoint 7's round/mode logic should ever decide whether score resets, not the respawn action itself. Position reset is a separate call to `PlayerController.setSpawn()` from `main.ts`, since `PlayerState` has no reason to know about camera/world position.
...
- **Round-based score reset**: whether/when `score` resets (new round? new game? never?) is a rule for whatever game mode owns rounds, decided in checkpoint 7 — `PlayerState.respawn()` deliberately never touches it in the meantime.
