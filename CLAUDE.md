...
src/
  ...
  core/
    ...
    WeaponSystem.ts             [2, addTarget()/removeTarget() added at 7 for dynamic per-enemy raycast targets]
    InteractSystem.ts           [3]
  ...
  modes/
    GameMode.ts                 [8]
    ZombieSurvival.ts           [7, owns round number + enemy spawn/despawn lifecycle]
    ShootingRange.ts            [8]
  state/
    GameState.ts                [1]
  ui/
...
- `WeaponSystem.addTarget()`/`removeTarget()` were added (checkpoint 7) alongside the existing `setTargets()`: `setTargets()` still sets the static base list (walls, doors, static interactables) once at boot, but a dynamically spawned/despawned object like a `ZombieSurvival` enemy needs to join and leave the raycast target list on its own schedule. Each `EnemyAI` instance calls `addTarget(this.mesh)` in its constructor and `removeTarget(this.mesh)` in `destroy()`, so `WeaponSystem` never needs to know about rounds or spawning.
...
- `GameState.currentRound` is mirrored from `ZombieSurvival.currentRound` every time `startRound()` runs, following the established "systems write to GameState, HUD only reads" pattern — same reasoning as `WeaponSystem` writing ammo every frame. `GameState.roundsSurvived` is a separate field, deliberately not the same as the live `currentRound`: it's snapshotted once, in `main.ts`'s `onDeath` callback, at the exact moment of death, so the death panel's number can't silently change if a round happens to advance in the background before the player clicks Respawn.
